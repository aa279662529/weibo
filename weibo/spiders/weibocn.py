...
class WeibocnSpider(Spider):
    ...
    fan_url = 'https://m.weibo.cn/api/container/getIndex?containerid=231051_-_fans_-{uid}&page={page}'
    ...
    def parse_follows(self,response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')) and result.get('data').get('crads')[-1].get('card_group'):
            #解析用户
            follows = result.get('data').get('crads')[-1].get('card_group')
